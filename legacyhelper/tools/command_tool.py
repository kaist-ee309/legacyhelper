...
def get_filtered_shell_history(ctx: RunContext[None], n: int = 10) -> str:
    ...
    def redact_line(text: str) -> str:
        ...
        sensitive_keys = r'(api[_-]?key|password|secret|token|credential|auth|passwd|pwd|private[_-]?key|bearer|authorization)'
        ...
        def replace_token(match):
            token = match.group(1)
            ctx = text[max(0, match.start()-20):min(len(text), match.end()+20)]
            if '/' in ctx or '://' in ctx or '@' in ctx: return token
            return '***REDACTED***'
